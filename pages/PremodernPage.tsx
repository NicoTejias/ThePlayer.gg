...
const PremodernPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                // Fetch Articles (Premodern category)
                const { data: articlesData } = await supabase
                    .from('articles')
                    .select('*')
                    .eq('is_published', true)
                    .ilike('category', '%Premodern%')
                    .order('created_at', { ascending: false });

                // Fetch Videos (mtg game type)
                ...
                if (articlesData) {
                    setArticles(articlesData.map(a => ({
                        ...
                        author: 'The Player',
                        excerpt: a.excerpt || '',
                        imageUrl: a.image_url || 'https://picsum.photos/seed/premodern/400/300',
                        category: a.category || 'Premodern'
                    })));
                }
                ...
                console.error('Error fetching premodern content:', error);
            } finally {
                setLoading(false);
            }
        };
                ...
                <>
                    <section>
                        ...
                        {articles.length > 0 ? (
                            ...
                            <div className="text-center py-10 bg-slate-800/50 rounded-xl border border-slate-700 border-dashed">
                                <p className="text-slate-400">Pronto tendremos artículos de Premodern.</p>
                            </div>
                        )}
                    </section>
                    ...
                    <section>
                        ...
                        {videos.length > 0 ? (
                            ...
                            <div className="text-center py-10 bg-slate-800/50 rounded-xl border border-slate-700 border-dashed">
                                <p className="text-slate-400">Pronto tendremos videos de Premodern.</p>
                            </div>
                        )}
                    </section>
                </>
